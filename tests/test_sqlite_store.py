...
from ypy_websocket.stores import SQLiteYStore, DocExists
...
def add_document():
    async def _inner(path: str, doc_path: str, version: int, data: bytes = b"") -> None:
        async with aiosqlite.connect(path) as db:
            await db.execute("INSERT INTO documents VALUES (?, ?)", (doc_path, version),)
            await db.execute("INSERT INTO yupdates VALUES (?, ?, ?, ?)", (doc_path, data, b"", time.time()),)
            await db.commit()
...
async def test_initialization(tmp_path):
    ...
    async with aiosqlite.connect(path) as db:
        ...
        cursor = await db.execute("SELECT count(*) FROM sqlite_master WHERE type='table' AND name='yupdates'")
        res = await cursor.fetchone()
        assert res[0] == 1
...
async def test_initialization_with_existing_database(tmp_path, create_database, add_document):
    ...
    async with aiosqlite.connect(path) as db:
        ...
        cursor = await db.execute("SELECT path, version FROM documents WHERE path = ?", (doc_path,),)
        res = await cursor.fetchone()
...
async def test_create(tmp_path, create_database, add_document):
    ...
    async with aiosqlite.connect(path) as db:
        cursor = await db.execute("SELECT path, version FROM documents WHERE path = ?", (new_doc,),)
        res = await cursor.fetchone()
